...
const Foodprovider = ({ children }) => {
  ...
  const getfoods = async () => {
    ...
    try {
      const res = await axios.get(`https://localhost:7281/api/pizzaItems`);
  ...
  const addfood = async (food) => {
    const addApi = "https://localhost:7281/api/pizzaItems";
    try {
      const res = await axios.post(addApi, food);
      const updatedfoods = await axios.get(
        `https://localhost:7281/api/pizzaItems`
      );
  ...
  const updatefood = async (id) => {
    const updateapi = `https://localhost:7281/api/pizzaItems/${id}`;
    try {
  ...
  const deletefood = async (id) => {
    const Dapi = `https://localhost:7281/api/pizzaItems/${id}`;
    try {
  ...
  return (
    <FoodContext.Provider value={{ ...state, deletefood, addfood, updatefood }}>
      {children}
